...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        place = Place.objects.get(title=new_place['title'])

        for img_num, img in enumerate(new_place['imgs']):

            Image.objects.get_or_create(place=place, position=img_num)
            image = Image.objects.get(place=place, position=img_num)
